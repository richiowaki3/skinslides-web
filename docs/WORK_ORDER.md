...
- Phase 1（core抽出）以降は未着手。

## 🔴 Phase 0 実行中に発見した最重要の別問題: R2 に動画が揃っていない

ブラウザ実機検証で判明。`logic_weights.json` は **81本**を参照するが、配信元の
Cloudflare R2 には **52本しか存在しない**（`01`〜`56` の範囲に大半、`57`以降は
ほぼ欠番。例: `61/64/69/70/71/90/91/92/93` などが 404）。

- これは分類やコードの問題ではなく**データ/デプロイの不整合**で、本修正以前から存在。
  旧分類の L4 プール(12本)にも欠番の 70/71/90/91/92/93 が含まれ、
  「最も再生される動画」の約半分が実際には 404 で黒画面/エラーになっていた。
- 再較正後はプールが 39本(L4)に広がり、可用な `01`〜`56` 帯が選ばれやすくなるため
  実効的には改善方向だが、**根治には次のどちらかが必要**:
  - (A) 起動時に全動画の HEAD 可用性を確認し、404 を videoPools から除外する
    （実装は容易・変種は52本に減る。simulate.js にも available フィルタを足す）
  - (B) 欠落している約29本を R2 にアップロードしてメタデータと一致させる（作者作業）
- **公開前に必須。** どちらで対応するかは作者判断。Phase 1 より優先度が高い。
...
### Phase 2 受け入れ基準
- [ ] `node tools/simulate.js planA new` → top10Share ≈ 22%（旧: 62%）
- [ ] `node tools/simulate.js planB new` → top10Share ≈ 14%（旧: 42%）
- [ ] unused = 0/81 を維持
- [ ] 実機で30分再生し、同一動画の目立った反復がないことを目視確認
